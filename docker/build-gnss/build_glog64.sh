...
#build armadillo
rm glog64 -rf
git clone https://github.com/google/glog.git glog64

cd glog64
git checkout v0.5.0
mkdir build
cd build
cmake -DBUILD_TESTING=false -DANDROID_TOOLCHAIN=${ANDROID_TOOLCHAIN} -DCMAKE_INSTALL_PREFIX=${PREFIX} -DCMAKE_FIND_ROOT_PATH=${PREFIX} -DCMAKE_TOOLCHAIN_FILE=${TOOLCHAIN_ROOT}/build/cmake/android.toolchain.cmake -DANDROID_ABI=${ANDROID_ABI} -DANDROID_ARM_NEON=ON -DANDROID_NATIVE_API_LEVEL=${NDKAPI} -DDETECT_HDF5=false  ../
make -j${NCORES}
make install
