...
#build armadillo
rm gflags64 -rf
git clone https://github.com/gflags/gflags.git gflags64

cd gflags64
git checkout v2.2.2
mkdir build
cd build
cmake -DGFLAGS_BUILD_SHARED_LIBS=true -DANDROID_TOOLCHAIN=${ANDROID_TOOLCHAIN} -DCMAKE_INSTALL_PREFIX=${PREFIX} -DCMAKE_FIND_ROOT_PATH=${PREFIX} -DCMAKE_TOOLCHAIN_FILE=${TOOLCHAIN_ROOT}/build/cmake/android.toolchain.cmake -DANDROID_ABI=${ANDROID_ABI} -DANDROID_ARM_NEON=ON -DANDROID_NATIVE_API_LEVEL=${NDKAPI} -DDETECT_HDF5=false  ../
make -j${NCORES}
make install
